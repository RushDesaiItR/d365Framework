...
(function (document, angular) {
    "use strict";
    var app = angular.module("MyApp", ["ngAnimate", "ui.bootstrap", "textAngular", "ngMaterial", "ngMessages", "angularjs-datetime-picker"]); //ngInputModified
    ...
    function DynamicController($scope, $http) {
                            ...
                            try {
                                for (var i = 0; i < result.length; i++) {
                                    for (var j in tabsdata) {
                                        var tab = tabsdata[j];
                                        if (tab.getName().toLowerCase() === result[i].tm_tabid.toLowerCase()) {
        ...
        function LoadOptionSetAndGetTabFieldsInQuery(tab, section) {
            ...
            //------------------------------------------------------------Added By Rushikesh To Set Range Input  ------------------------------------------------
            RangeUiSet()
            console.log("$scope.tabs.sections", $scope.tabs.sections)
        ...
        function RangeUiSet(){
            for (var k = 0; k < $scope.tabs.sections.length; k++) {
                ...
                for (var j = 0; j < fieldLength; j++) {
                    if ($scope.tabs.sections[k].fields[j].tm_fieldtype == 'Picklist') {
                         var rangeSelector = document.querySelector($scope.tabs.sections[k].fields[j].tm_fieldapiname);
                         rangeSelector.style.background = 'linear-gradient(90deg, #59a30f 60%,#f6f7f7 0%)'
        ...
        function GetOptionSetLabelMulti(tab, section, field, EntityLogicalName, AttributeLogicalName) {
            ...
            //        $scope.tabs.sections[section].fields[field].PickList.push({ id: multiOptionSet[i].value, value: multiOptionSet[i].text });
            //    }
            //}
